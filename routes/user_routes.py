...
def dashboard():
    ...
    return render_template('user/dashboard.html', 
                         projects=projects[:5],  # Show only 5 recent
                         estimates=estimates,
                         orders=orders,
                         stats=stats)
...
def create_project():
    ...
    if request.method == 'POST':
        ...
        # Save project and get the document reference
        doc_ref = db.collection('projects').add(project_data)
        project_id = doc_ref[1].id
        
        # Redirect to success page with estimation
